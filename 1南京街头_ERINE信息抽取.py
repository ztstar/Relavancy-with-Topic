...
# API Key和Secret Key
wenxin_api.ak = "0DR2REgLqKHU0aF3LnqBbXe208ERRzF6" #输入您的API Key
wenxin_api.sk = "Ymu3qY01yXQTW68Rw35hdbvilLI9CAOP" #输入您的Secret Key
...
def try_ask_erine(input_filename, output_filename):
    ...
    for article in articles:
        ...
        for question in questions:
            if not article['text'] or len(article['text']) == 0:
                raise Exception('文章必须都有摘要，而且还要是人工校对过的！')
            ...
            try:
                ...
                # 存盘
                save_articles(output_filename, articles)
                print('异常中止')
                return False
...
def main():
    try_ask_erine('南京街头100篇ocr清洗.jsonl', '1南京街头100篇ocr清洗_ERINE信息抽取.jsonl')
